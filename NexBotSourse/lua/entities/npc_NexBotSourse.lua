...
ENT.JumpSound = Sound("npc_NextBotSourse/jump.mp3")
ENT.JumpHighSound = Sound("npc_NextBotSourse/jump.mp3")
ENT.TauntSounds = {
	Sound("npc_NextBotSourse/attack.mp3")
}
local chaseMusic = Sound("npc_NextBotSourse/panic.mp3")

local workshopID = "1956391434"

local IsValid = IsValid

if SERVER then -- SERVER --

local npc_NextBotSourse_acquire_distance =
	CreateConVar("npc_NextBotSourse_acquire_distance", 9999, FCVAR_NONE,
	"The maximum distance at which NextBotSourse will chase a target.")

local npc_NextBotSourse_spawn_protect =
	CreateConVar("npc_NextBotSourse_spawn_protect", 0, FCVAR_NONE,
	"If set to 1, NextBotSourse will not target players or hide within 200 units of \z
	a spawn point.")

local npc_NextBotSourse_attack_distance =
	CreateConVar("npc_NextBotSourse_attack_distance", 120, FCVAR_NONE,
	"The reach of NextBotSourse's attack.")

local npc_NextBotSourse_attack_interval =
	CreateConVar("npc_NextBotSourse_attack_interval", 0.1, FCVAR_NONE,
	"The delay between NextBotSourse's attacks.")

local npc_NextBotSourse_attack_force =
	CreateConVar("npc_NextBotSourse_attack_force", 800, FCVAR_NONE,
	"The physical force of NextBotSourse's attack. Higher values throw things \z
	farther.")

local npc_NextBotSourse_smash_props =
	CreateConVar("npc_NextBotSourse_smash_props", 1, FCVAR_NONE,
	"If set to 1, NextBotSourse will punch through any props placed in their way.")

local npc_NextBotSourse_allow_jump =
	CreateConVar("npc_NextBotSourse_allow_jump", 1, FCVAR_NONE,
	"If set to 1, NextBotSourse will be able to jump.")

local npc_NextBotSourse_hiding_scan_interval =
	CreateConVar("npc_NextBotSourse_hiding_scan_interval", 3, FCVAR_NONE,
	"NextBotSourse will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant NextBotSourses are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_NextBotSourse_hiding_repath_interval =
	CreateConVar("npc_NextBotSourse_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to NextBotSourse's hiding spot will be redetermined every X seconds.")

local npc_NextBotSourse_chase_repath_interval =
	CreateConVar("npc_NextBotSourse_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of NextBotSourse's target will be redetermined every \z
	X seconds.")

local npc_NextBotSourse_expensive_scan_interval =
	CreateConVar("npc_NextBotSourse_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_NextBotSourse_force_download =
	CreateConVar("npc_NextBotSourse_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download NextBotSourse resources \z
...
if npc_NextBotSourse_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("NextBotSourse_nag")
util.AddNetworkString("NextBotSourse_navgen")
...
local VECTOR_NextBotSourse_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_NextBotSourse_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_NextBotSourse: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other NextBotSourses, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_NextBotSourse"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "NextBotSourseMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_NextBotSourse" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why NextBotSourse isn't working.
	net.Start("NextBotSourse_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_NextBotSourse: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_NextBotSourse: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_NextBotSourse: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_NextBotSourse: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_NextBotSourse: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, NextBotSourse in pairs(ents.FindByClass("npc_NextBotSourse")) do
		NextBotSourse:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "NextBotSourseNavGen", navEndGenerate)
	else
		print("npc_NextBotSourse: nav_generate failed to initialize")
...
concommand.Add("npc_NextBotSourse_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_NextBotSourse: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. NextBotSourse doesn't use
	...
	elseif isConsole then
		print("npc_NextBotSourse: Beginning nav_generate requested by server console")
	else
		return
	end
	...
	net.Start("NextBotSourse_navgen")
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_NextBotSourse_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_NextBotSourse_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_NextBotSourse_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_NextBotSourse_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. NextBotSourse means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_NextBotSourse_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_NextBotSourse_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_NextBotSourse_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_NextBotSourse_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_NextBotSourse_attack_distance:GetInt()
		...
		local repathInterval = npc_NextBotSourse_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_NextBotSourse_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_NextBotSourse_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_NextBotSourse_hiding_repath_interval:GetFloat()
...
local MAT_NextBotSourse = Material("npc_NextBotSourse/NextBotSourse")
killicon.Add("npc_NextBotSourse", "npc_NextBotSourse/killicon", color_white)
language.Add("npc_NextBotSourse", "NextBotSourse")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_NextBotSourse: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a NextBotSourse.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_NextBotSourse_music_volume =
	CreateConVar("npc_NextBotSourse_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by NextBotSourse. (0-1, where 0 is muted)")

-- If another NextBotSourse comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, NextBotSourses do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_NextBotSourse_PANIC_COUNT NextBotSourses are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of NextBotSourse.
local MUSIC_NextBotSourse_PANIC_COUNT = 8

local MUSIC_NextBotSourse_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_NextBotSourse_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_NextBotSourse") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("NextBotSoursePanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_NextBotSourse_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_NextBotSourse" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_NextBotSourse_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear NextBotSourse taunt us.
...
local function startTimer()
	if not timer.Exists("NextBotSoursePanicMusicUpdate") then
		timer.Create("NextBotSoursePanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_NextBotSourse)

	-- Get the normal vector from NextBotSourse to the player's eyes, and then compute
	...
	-- NextBotSourse should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("NextBotSourseHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("NextBotSourseHUDSmall", {
	font = "Arial",
	size = 24
})
...
local flavourTexts = {
	{
		"Gotta dance fast!",
		"Learning the Blinding dance!",
		"This is too easy to sing."
	}, {
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("NextBotSourse is studying this map.", "NextBotSourseHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "NextBotSourseHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "NextBotSourseHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "NextBotSourseHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "NextBotSourseHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("NextBotSourse_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "NextBotSourseNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. NextBotSourse doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_NextBotSourse_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for NextBotSourse to figure \z
		this map out.\n\z
...
net.Receive("NextBotSourse_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! NextBotSourse doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet NextBotSourse-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! NextBotSourse doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to NextBotSourse.\n\z
			\n\z
			If you ARE the server host, you can run npc_NextBotSourse_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not NextBotSourse-compatible!",
			"Ok", nil,
